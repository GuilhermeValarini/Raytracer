    ...
    unsigned y)
{
    ...
    while (true) {
        ...
        if ((sphere->transparency > 0 || sphere->reflection > 0) && depth < MAX_RAY_DEPTH) {
            ...
            refldir.normalize();
            //return (trace(phit + nhit * bias, refldir, spheres, depth + 1) * fresneleffect * sphere->surfaceColor) + sphere->emissionColor;
...
void render(Vec3f* image, unsigned width, unsigned height, const Sphere* spheres, const unsigned n)
{
    ...
    // Trace rays
    #pragma omp parallel for num_threads(nt) shared(image)
    for (unsigned y = 0; y < height; ++y) {
        for (unsigned x = 0; x < width; ++x) {
            trace(spheres, n, image, width, height, x, y);
        }
    }
}
